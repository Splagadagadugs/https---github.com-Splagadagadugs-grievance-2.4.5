...
const Form = () => {
  ...
  const handleSubmit = (e) => {
    if (firstName === "" && lastName ==="" && maidenName === "" && email === "" && studentNumber === "" && contactNumber === ""  && currentCollege === "" && currentYear === "" && classConcern === "" && natureConcern === "" && aboutConcern === "" && concern === "") console.log("missing field");
    else {
    e.preventDefault();
    setLoader(true)
...
useEffect(() => {
  ...
  if (studentNumber.length < MAX_LENGTH) {
    setErrorMessage("Up to 11 Numbers");
  } else if (studentNumber.length > MAX_LENGTH) {
...
useEffect(() => {
  ...
  if (contactNumber.length < MAX_LENGTH) {
    setErrorMessage3("Up to 11 Numbers");
  } else if (contactNumber.length > MAX_LENGTH) {
...
useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <TextField
        fullWidth
        error={studentNumber.length > MAX_LENGTH}
      ...
      <TextField
        fullWidth
        error = {contactNumber.length > MAX_LENGTH}
    ...
    >
      ...
      {/*MAIDEN NAME*/}
       <TextField
        fullWidth
        helperText={"Please enter your Maiden name"}
        label="Maiden Name"
        id="outlined-error"
        value={maidenName}
